...
static uint16_t icm40609_odr_to_hz(icm40609_odr_t odr)
{
    switch (odr) {
        case ICM40609_ODR_32KHZ:    return 32000;
        case ICM40609_ODR_16KHZ:    return 16000;
        case ICM40609_ODR_8KHZ:     return 8000;
        case ICM40609_ODR_4KHZ:     return 4000;
        case ICM40609_ODR_2KHZ:     return 2000;
        case ICM40609_ODR_1KHZ:     return 1000;
        case ICM40609_ODR_200HZ:    return 200;
        case ICM40609_ODR_100HZ:    return 100;
        case ICM40609_ODR_50HZ:     return 50;
        case ICM40609_ODR_25HZ:     return 25;
        case ICM40609_ODR_12_5HZ:   return 12;
        case ICM40609_ODR_6_25HZ:   return 6;
        case ICM40609_ODR_3_125HZ:  return 3;
        case ICM40609_ODR_1_5625HZ: return 1;
        case ICM40609_ODR_500HZ:    return 500;
        default:                    return 0;
    }
}
                                   ...
                                   icm40609_t *dev)
{
    ...
    if (dev) {
        ...
        dev->gyro_odr_hz = icm40609_odr_to_hz(odr);
    }
    return 2;
}
                                    ...
                                    icm40609_t *dev)
{
    ...
    if (dev) {
        ...
        dev->accel_odr_hz = icm40609_odr_to_hz(odr);
    }
    return 2;
}
                                  ...
                                  uint8_t *tx_buf)
{
     ...
     *   bit 0: INT1_POLARITY
     *
     * Leave INT2 at defaults (pulsed, open-drain, active low = 0x00).
     */
    uint8_t val = 0;
...
{
    /*
     * INTF_CONFIG1 (0x4D), reset value 0x91:
     *   bits 7:6: EN_TEST_MODE = 10 (must be set to 01 for normal op, but
     *             reset value has bit 7 set; datasheet says 01 for normal)
     *   bits 5:4: Reserved
     *   bit  3:   ACCEL_LP_CLK_SEL
     *   bit  2:   Reserved
     *   bits 1:0: CLKSEL
     *
     * We preserve the EN_TEST_MODE = 01 pattern and set CLKSEL.
     */
    uint8_t val = (0x01 << 6) | (clksel & 0x03);
...
/* Read FIFO data                                                             */
/* -------------------------------------------------------------------------- */
size_t icm40609_build_read_fifo_data(uint16_t byte_count, uint8_t *tx_buf)
{
    tx_buf[0] = SPI_RD(ICM40609_REG_FIFO_DATA);
    for (uint16_t i = 0; i < byte_count; i++)
        tx_buf[1 + i] = 0x00;
    return (size_t)(1 + byte_count);
}
...
bool icm40609_parse_fifo_count(const uint8_t *rx_buf, uint16_t *count)
{
    ...
    /*
     * FIFO count is in big-endian format by default:
     *   rx_buf[0] = FIFO_COUNTH (high byte)
     *   rx_buf[1] = FIFO_COUNTL (low byte)
     *
     * Note: FIFO_COUNTL must be read first to latch both registers.
     * The build function reads starting from FIFO_COUNTH, so auto-increment
     * gives us H first, then L.
     */
