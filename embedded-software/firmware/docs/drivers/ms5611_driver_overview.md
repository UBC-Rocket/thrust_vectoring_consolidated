...
4. **Ring buffer reset** – the `ms5611_sample_q_t` ring buffer (generated by `SPSC_RING_DECLARE` in `libs/collections/include/collections/spsc_ring.h`) head and tail pointers are zeroed so consumers start from a clean queue.
...
4. The sample is enqueued via `ms5611_sample_q_push(&ms5611_sample_ring, &sample)` for consumers to read later.
