...
/* USER CODE END Boot_Mode_Sequence_1 */
  ...
  /* Dual-core boot race (ROOT CAUSE of the silent VCP): on the H745 the system
   * clock is configured by CM7 (SystemClock_Config). With
   * DUAL_CORE_BOOT_SYNC_SEQUENCE disabled, CM4 runs standalone and would
   * otherwise initialise LPUART1 *before* CM7 finishes the clock setup — so the
   * UART baud is computed against the wrong kernel clock and nothing readable
   * reaches the ST-Link VCP. Spin briefly to let CM7 bring the clock up first.
   * This mirrors the known-good dual-core-logging bring-up (commit d5ba8b3);
   * replace with DUAL_CORE_BOOT_SYNC_SEQUENCE once the STOP-mode option bytes
   * are confirmed safe on hardware. */
  for (volatile uint32_t d = 0; d < 50000000U; ++d) {
    __NOP();
  }
  /* USER CODE END Init */
